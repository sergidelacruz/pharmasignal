...
def generate_sales():
    ...
    for drug_id, base_units in MONTHLY_UNITS.items():
        ...
        rows.append({
            "drug_id": drug_id,
            "units_sold": round(base_units * noise),
        })
...
def generate_adverse_events(n=2000):
    rows = []
    for i in range(1, n + 1):
        ...
        if is_flagged and random.random() < 0.38:
        ...
        rows.append({
            ...
            "event_date": date(2022, 1, 1) + timedelta(days=random.randint(0, 730)),
            "reaction": reaction,
            "is_novel": is_novel,               
            "outcome": outcome,
